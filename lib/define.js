...
function define (src, dest, options = {}) {
  const {
    cacheFile = `tmp/cache/${path.basename(dest, path.extname(dest))}-cache-:version-:env.json`,
    debug = !isProduction(),
    watchDelay = 100,
    ignores = [],
    noParse = [],
  ...
  const fileNameForEnv = (filename) => filename
    .replace(':version', version || '')
    .replace(':env', process.env.NODE_ENV || 'default')
    .replace(/--+/g, '-')
...
define.all = (srcDir, destDir, options = {}) => {
  const {pattern = '+(*.jsx|*.js|*.mjs)', skipWatching} = options
  const defined = async () => {
    const filenames = await aglob(pattern, {cwd: srcDir})
    return filenames.map((filename) => {
      const src = path.join(srcDir, filename)
      const dest = path.join(destDir, filename)
      return define(src, dest, options)
    })
  }
